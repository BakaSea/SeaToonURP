...
real W_f(real x, real e0, real e1)
{
    if (x <= e0)
        return 0;
    if (x >= e1)
...
real H_f(real x, real e0, real e1)
{
    if (x <= e0)
        return 0;
    if (x >= e1)
...
real GranTurismoTonemap(real x)
{
    real P = 1; // Maximum brightness
    real a = 1; // Contrast
    real m = 0.22; // Linear section start
    real l = 0.4; // Linear section length
    real c = 1.33; // Black pow  def 1 
    real b = 0; // Black min
    real l0 = (P - m) * l / a;
    real L0 = m - m / a;
    real L1 = m + (1 - m) / a;
    real L_x = m + a * (x - m);
    real T_x = m * pow(x / m, c) + b;
    real S0 = m + l0;
    real S1 = m + a * l0;
    real C2 = a * P / (P - S1);
    real S_x = P - (P - S1) * exp(-(C2 * (x - S0) / P));
    real w0_x = 1 - W_f(x, 0, m);
    real w2_x = H_f(x, m + l0, m + l0);
    real w1_x = 1 - w0_x - w2_x;
    real f_x = T_x * w0_x + L_x * w1_x + S_x * w2_x;
    return f_x;
}

real3 GTTonemap(real3 x)
{
    return real3(GranTurismoTonemap(x.x), GranTurismoTonemap(x.y), GranTurismoTonemap(x.z));
}
